...
var DateTime = (function () {
     ...
     *
     * 例子：
     *      Ares.DateTime.format("yyyy-MM-dd hh:mm:ss.S") ==> 2006-07-02 08:09:04.423
     *      Ares.DateTime.format("yyyy-M-d h:m:s.S")      ==> 2006-7-2 8:9:4.18
     *
     * @param d -- Date 对象
     * @param p -- 格式控制
     * @returns {*|string} -- 转换结果
    ...
    var _lastMonth = function(dateParams,lastNum){
        lastNum = lastNum || 1;
        var date = new Date();
        if(dateParams){
        ...
        var year = date.getFullYear();
        var month = date.getMonth() + 1 -lastNum;
        if(month<1){
            year = year-1;
            month = 12 - month
        }
        var day = Math.min(DateTime.getDays(year, month), date.getDate())
        return year + "-" + (month < 10 ? "0" + month : month)
            + "-" + (day < 10 ? "0" + day : day);
    }
